...
def punchy():
    ...
    while True:
        ...
        inputs = input("\nPlease input your instructions, each character separated by a space: ")

        #Checks if the user put spaces in between the characters
        if len(inputs) >= 2:
            if not inputs[1].isspace():
                print("\nInvalid input. Make sure to add spaces in between each character.")
                return again()
            if len(inputs)> 3:
                if not inputs[3].isspace():
                    print("\nInvalid input. Make sure to add spaces in between each character.")
                    return again()    

        #Splits the inputed characters into a list
